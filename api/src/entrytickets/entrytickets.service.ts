...
export class EntryticketsService {
  ...
  async getAll(): Promise<any> {
    const url = `${process.env.ENDPOINT}/entry-tickets`;
  ...
  async getById(id: string): Promise<any> {
    const url = `${process.env.ENDPOINT}/entry-tickets/${id}`;
    console.log(`Resend to ${url}`);
    return this.httpService.get(url).pipe(
      map((response: AxiosResponse) => response.data),
      catchError((error: AxiosError) => {
  ...
  async postCreate(body: EntryTicketReqDto) {
    const url = `${process.env.ENDPOINT}/entry-tickets`;
    console.log(`Resend to ${url}`);
  ...
  async patchModify(id: string, body: EntryTicketReqDto) {
    const url = `${process.env.ENDPOINT}/entry-tickets/${id}`;
    console.log(`Resend to ${url}`);
